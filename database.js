...
export function initializeDB() {
  if (!load() || !_DB.chats || _DB.chats.length === 0) {
    console.log("No local DB found or chats are missing, seeding initial data.");
    _DB = JSON.parse(JSON.stringify(initialData));

    // Generate chats
    const users = _DB.users;
    const owners = users.filter(u => [UserRole.Owner, UserRole.CoOwner].includes(u.role)).map(u => u.id);
    const managementAndDispatch = users.filter(u => [UserRole.Secretary, UserRole.Dispatch].includes(u.role)).map(u => u.id);
    const opsManagers = users.filter(u => [UserRole.OperationsManager, UserRole.OperationsDirector].includes(u.role)).map(u => u.id);
    const supervisors = users.filter(u => [UserRole.Supervisor].includes(u.role)).map(u => u.id);
    const trainingOfficers = users.filter(u => [UserRole.TrainingOfficer].includes(u.role)).map(u => u.id);
    const guards = users.filter(u => [UserRole.Guard, UserRole.SiteLead].includes(u.role)).map(u => u.id);

    const allManagement = [...new Set([...owners, ...managementAndDispatch, ...opsManagers, ...supervisors, ...trainingOfficers])];

    const defaultChats = [
        { id: 'chat-guards', name: 'Guard Chat', members: [...new Set([...guards, ...supervisors, ...trainingOfficers, ...managementAndDispatch, ...owners])] },
        { id: 'chat-tos', name: 'Training Officer Chat', members: [...new Set([...trainingOfficers, ...guards, ...supervisors, ...managementAndDispatch])] },
        { id: 'chat-sups', name: 'Supervisor Chat', members: [...new Set([...supervisors, ...guards, ...trainingOfficers, ...managementAndDispatch])] },
        { id: 'chat-ops', name: 'Operations Manager Chat', members: [...new Set([...opsManagers, ...guards, ...supervisors, ...managementAndDispatch])] },
        { id: 'chat-mgmt', name: 'Management (Dispatch) Chat', members: [...new Set([...managementAndDispatch, ...guards, ...supervisors, ...owners])] },
        { id: 'chat-owners', name: 'Owner Chat', members: [...new Set([...owners, ...supervisors, ...trainingOfficers, ...managementAndDispatch])] },
        { id: 'chat-g-to-to', name: 'Guard-to-Training Officer', members: [...new Set([...guards, ...trainingOfficers])] },
        { id: 'chat-g-to-sup', name: 'Guard-to-Supervisor', members: [...new Set([...guards, ...supervisors])] },
        { id: 'chat-g-to-ops', name: 'Guard-to-Operations Manager', members: [...new Set([...guards, ...opsManagers])] },
        { id: 'chat-g-to-mgmt', name: 'Guard-to-Management', members: [...new Set([...guards, ...managementAndDispatch])] },
        { id: 'chat-g-to-owner', name: 'Guard-to-Owner', members: [...new Set([...guards, ...owners])] },
    ];
    
    _DB.chats = defaultChats.filter(c => c.members.length > 1).map(c => ({...c, isGroup: true}));
    
    if (_DB.chats.length > 0 && _DB.messages.length === 0) {
        _DB.messages.push({ id: 'msg-1', chatId: 'chat-guards', senderId: owners[0] || allManagement[0], content: 'Welcome to the Guard Chat. All guards can communicate here. Supervisors, Training Officers, and Management can see and respond to messages.', timestamp: new Date(Date.now() - 1000 * 60 * 5) });
    }
...
export const getChatsForUser = (user) => {
    if (!user) return [];
    const allChats = getCollection('chats');
    const { role, teamId } = user;

    const managementAndOwnerRoles = [UserRole.Owner, UserRole.CoOwner, UserRole.Secretary, UserRole.Dispatch];
    if (managementAndOwnerRoles.includes(role)) {
        return allChats;
    }
    
    const operationsRoles = [UserRole.OperationsDirector, UserRole.OperationsManager];
    if (operationsRoles.includes(role)) {
        const teamUsers = new Set(getUsers().filter(u => u.teamId === teamId).map(u => u.id));
        return allChats.filter(chat => chat.members.some(memberId => teamUsers.has(memberId)));
    }
    
    return allChats.filter((chat) => chat.members.includes(user.id));
};
