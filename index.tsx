...
function openModal(type: string, id: any = null) {
...
function handleFilterMissions(status: string) {
    state.missionControlFilter = status;
    render();
}

function handleFilterContracts(status: string) {
...
function handleEditContract(e: Event) {
    e.preventDefault();
    const form = e.target as HTMLFormElement;
...
function handlePromoteDemoteSubmit(e: Event) {
    e.preventDefault();
    const form = e.target as HTMLFormElement;
...
function handleCreateGuardSubmit(e: Event) {
    e.preventDefault();
    if (!state.currentUser) return;
    const form = e.target as HTMLFormElement;
    const data = getFormData(form);
...
function handleCreateClientSubmit(e: Event) {
    e.preventDefault();
    const form = e.target as HTMLFormElement;
    const data = getFormData(form);
...
function handleSystemSettingsSubmit(e: Event) {
    ...
    if (state.currentUser.role === UserRole.Owner) {
        const commissionRates: { [key: string]: number } = { ...getSystemSettings().commissionRates };
...
function handleAppealSubmit(e: Event) {
    e.preventDefault();
    if (!state.currentUser) return;
    const form = e.target as HTMLFormElement;
    const missionId = form.dataset.missionId;
...
function handleReassignGuardSubmit(e: Event) {
    e.preventDefault();
    const form = e.target as HTMLFormElement;
    const missionId = form.dataset.missionId;
    if (!missionId) return;
...
function handleAssignVehicleSubmit(e: Event) {
    e.preventDefault();
    const form = e.target as HTMLFormElement;
    const data = getFormData(form);
...
function handleMessageSubmit(e: Event) {
    ...
    const form = e.target as HTMLFormElement;
    const data = getFormData(form);
    if (!data.content || (data.content as string).trim() === '') return;
...
function handleAdjustPay(e: Event) {
    e.preventDefault();
    alert('Pay adjustment functionality not implemented.');
    closeModal();
}
function handleDispatchGuard(e: Event) {
    e.preventDefault();
...
function attachEventListeners() {
    root.addEventListener('click', (e) => {
        ...
        if (!target) {
            const isClickInsideDropdown = (e.target as Element).closest('.actions-dropdown');
        ...
        const actionMap: { [key: string]: (e: Event) => void } = {
            ...
            'close-modal-backdrop': (e) => { if (!(e.target as Element).closest('[data-modal-content]')) closeModal(); },
            'close-mobile-menu': (e) => { if (!(e.target as Element).closest('[data-menu-panel]')) { state.isMobileMenuOpen = false; render(); }},
            ...
            'toggle-guard-actions': () => {
                const button = (e.target as Element).closest('[data-action="toggle-guard-actions"]');
                if (!button) return;
            ...
            'toggle-client-actions': () => {
                const button = (e.target as Element).closest('[data-action="toggle-client-actions"]');
                if (!button) return;
                const dropdown = button.closest('.actions-dropdown')?.querySelector('.actions-dropdown-content');
                if (!dropdown) return;
    ...
    root.addEventListener('input', (e) => {
        ...
        if (target.id === 'client-search-input') {
            handleClientSearch((target as HTMLInputElement).value);
        }
