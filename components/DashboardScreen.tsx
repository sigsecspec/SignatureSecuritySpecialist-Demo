...
import { CreateGuardModal } from './CreateGuardModal.js';
import { PromoteDemoteModal } from './PromoteDemoteModal.js';
...
export const DashboardScreen = ({ currentUser, activeView, activeMissionId, selectedPayrollRunId, selectedModal, isMobileMenuOpen, missionControlFilter, contractManagementFilter, activeChatId }: any) => {
    let viewContent = '';
        ...
        const viewMap: { [key: string]: () => string } = {
            ...
            'ContractManagement': () => {
                ...
                const filteredContracts = allContracts.filter(c => {
                    ...
                    if (contractManagementFilter === 'Pending') return c.status === 'Pending' || c.status === 'Ready for Review';
                    if (contractManagementFilter === 'Expired') return c.status === 'Active' && new Date(c.endDate) < now;
                    if (contractManagementFilter === 'Terminated') return c.status === 'Terminated' || c.status === 'Denied';
                    return false;
                });
